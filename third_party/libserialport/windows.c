...
{
	_locale_t locale = getLocale();

	size_t wc_length = wc_bytes / sizeof(WCHAR);
	size_t utf8_bytes;
	WCHAR* wc_str = NULL;
	char* utf8_str = NULL;
	...
	const size_t _buffer_size = utf8_bytes;
	err = _wcstombs_s_l(&utf8_bytes, utf8_str, _buffer_size, wc_str, _buffer_size - 1, locale);
...
SP_PRIV enum sp_return list_ports(struct sp_port*** list)
{
			...
			NULL, &type, (LPBYTE)data, &data_size) == ERROR_SUCCESS)
	{
		if (type == REG_SZ) {
			...
			/*errno_t err = */_wcstombs_s_l(&name_len, NULL, 0, data, 0, locale);
			/////////if ( ((__crt_locale_data_public*)(locale)->locinfo)->_locale_lc_codepage )
			...
			const size_t _buffer_size = name_len;
			/*err = */_wcstombs_s_l(&name_len, name, _buffer_size, data, _buffer_size - 1, locale);
			if (locale) free(locale);
			locale = NULL;
#else
			strcpy(name, data);
